...
export class SnippetComponent implements OnInit {
  ...
  constructor(
    ...
    private cdr: ChangeDetectorRef // Inietta il servizio dei gruppi
  ) {}

  ngOnInit() {
    this.userId = this.authService.getUserIdFromToken();
    console.log("id user E' : ", this.userId);
    ...
    console.log('gruppi caricati : ', this.groups);
  ...
  loadGroups() {
    this.groupService.getGroupsByUserId(this.userId).subscribe((data) => {
      this.groups = data;
      console.log("gruppi caricati : ", this.groups);  // Ora i gruppi sono caricati, puoi stamparli
      if (this.groups.length > 0) {
        this.selectedGroup = this.groups[0];
        console.log("Gruppo selezionato: ", this.selectedGroup);
        this.filterSnippetsByGroup(this.selectedGroup.idGroup)
        console.log("gruppi caricati correttammente");
      } else {
  ...
  selectGroup(group: any) {
    ...
    this.cdr.detectChanges(); // Forza l'aggiornamento del template
  ...
  async filterSnippetsByGroup(groupId: number) {
    this.snippetService.getSnippetsByUserId(this.userId).subscribe({
      next: (data) => {
        this.snippets = data;
        console.log("GLI SNIPPETS: ", this.snippets)
...
.filter((snippet: { idGroup: { idGroup: number } }) => 
          ...
          console.log("this.filteredSnippets",this.filteredSnippets)
  ...
  deleteSnippet(): void {
    if (this.snippetToDeleteId !== null) {
      this.snippetService
        .deleteSnippetsByUserId(this.userId, this.snippetToDeleteId)
        .subscribe({
          next: () => {
            ...
            if (
              this.filteredSnippets.length <=
                (this.currentPage - 1) * this.snippetsPerPage &&
              this.currentPage > 1
            ) {
              this.currentPage--;
            }
          ...
          error: (error: any) => {
            console.error(
              "Errore durante l'eliminazione dello snippet:",
              error
            );
          },
        });
  ...
  getLanguageForSnippet(snippet: any): string {
    ...
    if (
      ...
      snippet.content.includes('public') ||
      snippet.content.includes('void')
    ) {
      return 'java'; // Se è codice Java
    } else if (
      snippet.content.includes('def ') ||
